...
func (g *NodeEdge) DagShortestPaths(start string) {
	linearOrder := g.TopoSort() //Set linearOrder to be the linear order returned

	for i := 0; i < len(linearOrder); i++ {
		g.shortest[linearOrder[i]] = UNLIMITED //Set shortest for each vertices(except start) to unlimited
		g.pred[linearOrder[i]] = ""            //Set pred for each vertex to empty("")
	}
